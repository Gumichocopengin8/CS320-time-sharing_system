...
with open('test.txt','r') as file:
    ...
    while not tasks.empty(): # while have processes
        ...
        if process.time > 0: # if process is not done
            ...
            output += ("{0} \t{1} removed from the processor.".format(CPUTime, process.name) + " Time left for process: {0}\n".format(process.time))
        else:
            output += ("{0} \t{1} removed from the processor.".format(CPUTime, process.name) + " {0} is complete.\n".format(process.name))
